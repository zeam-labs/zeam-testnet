...
func (cc *CollapseCompute) SetFeed(feed node.NetworkFeed) {
	cc.mu.Lock()
	defer cc.mu.Unlock()
	cc.feed = feed
}

func (cc *CollapseCompute) BroadcastCollapse(tx *types.Transaction) <-chan node.BroadcastResult {
	cc.mu.RLock()
	r := cc.feed
	cc.mu.RUnlock()
	if r == nil {
		ch := make(chan node.BroadcastResult, 1)
		ch <- node.BroadcastResult{Err: nil}
		close(ch)
		return ch
	}
	return r.BroadcastTx(tx)
}
