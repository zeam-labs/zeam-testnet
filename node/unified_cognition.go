...
func NewUnifiedCognition(transport *HybridTransport) *UnifiedCognition {
	harvester := NewFlowHarvester(transport)
...
func (uc *UnifiedCognition) Start() error {
	...
	if err := uc.harvester.Start(); err != nil {
		return err
	}

	
	if uc.transport != nil && uc.transport.devp2p != nil {
		devp2p := uc.transport.devp2p
...
func (uc *UnifiedCognition) cognitionLoop() {
	...
	for {
		...
		case <-ticker.C:
			uc.processCycle(&lastFlowCount)
		}
	}
}


func (uc *UnifiedCognition) processCycle(lastFlowCount *uint64) {
	uc.mu.Lock()
	defer uc.mu.Unlock()

	
	flow := uc.harvester.Flow()
	if flow != nil {
		count, _, _ := flow.Stats()
		if count > *lastFlowCount {
			newCount := int(count - *lastFlowCount)
			uc.detector.RecordTxBatch(newCount)
			*lastFlowCount = count

			
			flowEntropy := flow.Entropy()
			for i := 0; i < 32; i++ {
				uc.entropyPool[i] ^= flowEntropy[i]
			}
			uc.poolVersion++
		}
	}
...
func (uc *UnifiedCognition) processWork(work WorkItem) {
	...
	hash, err := uc.transport.BroadcastZEAM(mixedPayload)
	if err != nil {
		return
	}

	
	uc.ingestBroadcastResult(hash, work.Concepts)
	...
	if work.Callback != nil {
		work.Callback([]common.Hash{hash})
	}
}
...
func (uc *UnifiedCognition) ingestBroadcastResult(hash common.Hash, concepts []string) {
	...
	for _, concept := range concepts {
		uc.concepts[concept] = min(1.0, uc.concepts[concept]+0.3)
	}
...
func (uc *UnifiedCognition) InjectUserInput(text string, concepts []string) {
	...
	for _, concept := range concepts {
		uc.concepts[concept] = min(1.0, uc.concepts[concept]+0.8)
	}

	
	uc.dynamics.UserActivity = min(1.0, uc.dynamics.UserActivity+0.5)
...
func (uc *UnifiedCognition) Steer(concepts []string, strength float64) {
	...
	for _, concept := range concepts {
		uc.concepts[concept] = min(1.0, uc.concepts[concept]+strength)
	}
}
