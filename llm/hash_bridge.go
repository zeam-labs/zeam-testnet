...
func (b *HashBridge) indexSynsets() error {
	...
	fmt.Printf("[HASH-BRIDGE] Indexed synsets: n=%d v=%d adj=%d adv=%d\n",
		b.synsetCounts["n"], b.synsetCounts["v"],
		b.synsetCounts["adj"], b.synsetCounts["adv"])
...
func (b *HashBridge) BuildBoundsFromConcepts(concepts []string) *quantum.SemanticBounds {
	...
	bounds := &quantum.SemanticBounds{
		...
		ValidCategories: []string{"n", "v", "adj", "adv"},
		MinCoherence:    0.3,
		MaxTension:      0.8,
...
func (b *HashBridge) BuildBoundsFromPressure(baseConcepts []string) *quantum.SemanticBounds {
	...
	if pressure.Coherence > 0.7 {
		bounds.MinCoherence = 0.5
	}

	
	if pressure.Tension > 0.6 {
		bounds.MaxTension = 0.9
	}

	
	if pressure.Magnitude > 0.7 {
		bounds.ValidCategories = []string{"v", "n", "adj", "adv"}
	}

	
	if pressure.Density < 0.3 {
...
func assembleCoherent(words []string, pressure quantum.PressureMetrics) string {
	...
	if pressure.Coherence > 0.7 && len(words) > 3 {
		words = words[:3]
	}

	
	result := strings.Join(words, " ")
	if pressure.Magnitude > 0.8 {
