...
func blendPressure(old, new quantum.PressureMetrics, alpha float64) quantum.PressureMetrics {
	return quantum.PressureMetrics{
		Magnitude: old.Magnitude*(1-alpha) + new.Magnitude*alpha,
		Coherence: old.Coherence*(1-alpha) + new.Coherence*alpha,
		Tension:   old.Tension*(1-alpha) + new.Tension*alpha,
		Density:   old.Density*(1-alpha) + new.Density*alpha,
	}
}
...
func (b *NGACBridge) OnChainEvent(eventType string, amount float64, metadata string) {
	...
	b.lastPressure = blendPressure(b.lastPressure, eventPressure, 0.15)
